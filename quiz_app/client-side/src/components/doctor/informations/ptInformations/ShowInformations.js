import React, { useState, useEffect } from 'react';
import { connect } from 'react-redux';
import { bindActionCreators } from 'redux';

import * as userActions from '../../../../redux/actions/userActions';
import { useLocation } from 'react-router-dom';
import { Col, Container, Row } from 'reactstrap';
import UserDropdown from '../../../user/UserDropdown';

import * as userTypes from '../../../user/UserTypes';

function ShowInformations(props) {
    const { actions, user, patient, email, role } = props;
    const [isLogadingUserInfo, setIsLogadingUserInfo] = useState(true);

    useEffect(() => {
        actions.getUserInfo(user.id)
            .then(() => {
                setIsLogadingUserInfo(false);
            })
            .catch((error) => {
                console.log(error);
                setIsLogadingUserInfo(false);
            })
    }, [actions, user]);

    if (isLogadingUserInfo) {
        return (
            <div>
                Loading...
            </div>
        );
    }

    let informationHeader;

    if (role.toUpperCase() === userTypes.PARENT) {
        informationHeader = (
            <div className='ui label'>
                Ebeveyn Bilgileri
            </div>
        );
    } else if (role.toUpperCase() === userTypes.TEACHER) {
        informationHeader = (
            <div className='ui label'>
                Öğretmen Bilgileri
            </div>
        );
    }


    return (
        <div>
            <Container>
                <Row>
                    <Col xs='12'>
                        <UserDropdown />
                    </Col>
                </Row>
            </Container>


            {/* patient information */}
            <Container fluid className='p-2'>
                <Row>
                    <Col xs='12'>
                        <div className='ui black huge labels'>
                            {informationHeader}
                        </div>
                    </Col>
                </Row>
            </Container>
            <br></br>
            <Container>
                {/* parent/teacher number*/}
                <Row>
                    <Col xs='4'>
                        <div className='ui black huge labels'>
                            <div className='ui label'>
                                Numarası:
                            </div>
                        </div>
                    </Col>
                    <Col xs='auto'>
                        <div className='ui blue huge labels'>
                            <div className='ui label'>
                                {user.id}
                            </div>
                        </div>
                    </Col>
                </Row>

                {/* parent/teacher name */}
                <Row>
                    <Col xs='4'>
                        <div className='ui black huge labels'>
                            <div className='ui label'>
                                Adı:
                            </div>
                        </div>
                    </Col>
                    <Col xs='auto'>
                        <div className='ui blue huge labels'>
                            <div className='ui label'>
                                {user.name}
                            </div>
                        </div>
                    </Col>
                </Row>

                {/* parent/teacher surname */}
                <Row>
                    <Col xs='4'>
                        <div className='ui black huge labels'>
                            <div className='ui label'>
                                Soyadı:
                            </div>
                        </div>
                    </Col>
                    <Col xs='auto'>
                        <div className='ui blue huge labels'>
                            <div className='ui label'>
                                {user.surname}
                            </div>
                        </div>
                    </Col>
                </Row>

                {/* parent/teacher email */}
                <Row>
                    <Col xs='4'>
                        <div className='ui black huge labels'>
                            <div className='ui label'>
                                Email:
                            </div>
                        </div>
                    </Col>
                    <Col xs='auto'>
                        <div className='ui blue huge labels'>
                            <div className='ui label'>
                                {email}
                            </div>
                        </div>
                    </Col>
                </Row>

                {/* related patient name surname*/}
                <Row>
                    <Col xs='4'>
                        <div className='ui black huge labels'>
                            <div className='ui label'>
                                İlgili Hasta:
                            </div>
                        </div>
                    </Col>
                    <Col xs='auto'>
                        <div className='ui blue huge labels'>
                            <div className='ui label'>
                                {patient.name + ' ' + patient.surname}
                            </div>
                        </div>
                    </Col>
                </Row>

                {/* related patient number*/}
                <Row>
                    <Col xs='4'>
                        <div className='ui black huge labels'>
                            <div className='ui label'>
                                İlgili Hasta Numarası:
                            </div>
                        </div>
                    </Col>
                    <Col xs='auto'>
                        <div className='ui blue huge labels'>
                            <div className='ui label'>
                                {patient.id}
                            </div>
                        </div>
                    </Col>
                </Row>
            </Container>


        </div>
    );
}

// getting props
function mapStateToProps(state) {
    return {
        // user information detail
        role: state.userReducer.role,
        email: state.userReducer.email,

        // base user id
        doctorId: state.authReducer.userId,
    };
}

// dispatch actions
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators({ ...userActions }, dispatch),
    };
}

const ConnectedShowInformations = connect(
    mapStateToProps,
    mapDispatchToProps
)(ShowInformations);

export default function ShowInformationsClass() {
    const location = useLocation();
    const { user, patient, } = location.state;

    return <ConnectedShowInformations user={user} patient={patient} />;
}
